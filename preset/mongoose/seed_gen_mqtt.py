...
def generate_files(seeds_dir, output_dir):
    ...
    mqtt_gen_code += "    try:\n"
    mqtt_gen_code += "        g = globals()\n"
    mqtt_gen_code += "        funcs = []\n"
    mqtt_gen_code += "        this_lineno = __mqtt_gen__.__code__.co_firstlineno\n"
    mqtt_gen_code += "        for name, obj in g.items():\n"
    mqtt_gen_code += "            if callable(obj) and hasattr(obj, '__module__') and obj.__module__ == __name__:\n"
    mqtt_gen_code += "                 if hasattr(obj, '__code__') and obj.__code__.co_firstlineno < this_lineno:\n"
    mqtt_gen_code += "                     funcs.append(obj)\n"
    mqtt_gen_code += "        funcs.sort(key=lambda f: f.__code__.co_firstlineno)\n"
    mqtt_gen_code += "        for i, func in enumerate(funcs):\n"
    mqtt_gen_code += "            try:\n"
    mqtt_gen_code += "                f.write(func())\n"
    mqtt_gen_code += "            except Exception:\n"
    mqtt_gen_code += "                pass\n"
    mqtt_gen_code += "    except Exception:\n"
    mqtt_gen_code += "        pass\n"
