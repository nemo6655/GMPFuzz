#!/usr/bin/env python3
"""
driver_net.py - Execute individual MQTT generator functions in a sandboxed
environment. Converts Python variant scripts into raw binary output files.

This is the core execution engine: given a Python module containing MQTT
packet generator functions (with a __mqtt_gen__ dispatcher), it runs them
in a sandboxed environment with timeout and memory limits, producing raw
binary output files that can be fed to aflnet.
"""
...
class MQTTWrapper:
    """Wrapper for MQTT generator modules that have individual packet functions."""
...
def generate_one(
    ...
    """Execute a generator function once, producing a single output file."""
...
def get_function(module_path, function_name, args):
    """Load a Python module and extract the generator function."""
    try:
        with Sandbox(args.timeout, args.max_mem) as s:
            ...
            try:
                ...
                # If function not found, check if we should use the MQTTWrapper
                func_names = [k for k, v in sorted(generator_module.__dict__.items())
                              if k.startswith('seed_') and callable(v)]
                if func_names:
                    return MQTTWrapper(func_names)
                raise
...
def make_parser(description='Run an MQTT input generator function'):
    ...
    parser.add_argument('-S', '--size-limit', type=int, default=50 * 1024,
                        help='Maximum size of the output file (in bytes)')
    parser.add_argument('-o', '--output-prefix', type=str, default='./output',
                        help='Output prefix')
    parser.add_argument('-s', '--output-suffix', type=str, default='.raw',
                        help='Output suffix')
    parser.add_argument('-t', '--timeout', type=int, default=10,
                        help='Timeout for the run (in seconds)')
    parser.add_argument('-M', '--max-mem', type=int, default=1024 * 1024 * 1024,
