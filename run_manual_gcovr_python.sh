...
for MODE in "${MODES[@]}"; do
    ...
    if [ -z "$EVAL_DIR" ]; then echo "EVAL_DIR NOT FOUND for $MODE"; continue; fi
    
    LAST_GEN=$(ls -d ${EVAL_DIR}/gen*/ 2>/dev/null | sort -rV | head -n 1)
    if [ -n "$LAST_GEN" ]; then
        if ! ls "$LAST_GEN/aflnetout"/aflnetout_*.tar.gz 1> /dev/null 2>&1; then
            LAST_GEN=$(ls -d ${EVAL_DIR}/gen*/ 2>/dev/null | sort -rV | sed -n '2p')
        fi
    fi
    
    if [ -z "$LAST_GEN" ] || ! ls "$LAST_GEN/aflnetout"/aflnetout_*.tar.gz 1> /dev/null 2>&1; then
    ...
    cp aflnet_replay.py "$MERGED_REPLAY_DIR/"
...
for f in /mnt/inputs/*; do
     ...
     python3 /mnt/inputs/aflnet_replay.py "$f" > /dev/null 2>&1 || true
